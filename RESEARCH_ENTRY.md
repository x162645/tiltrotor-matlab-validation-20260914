...
这些是条件数值结果：V4 静态源在既有 27 个点仍有约 33%–38% 误差，D16 仅新增了 Woodgate/NUAA 通用旋翼图示点的部件级动态检查（54 点，原始数组缺失），D16 升沉扩展为 PP-only 条件实验。不能称实机精度、全机验证或国内领先。下一精确接续是获取同步实际总距/执行器、旋翼载荷和升沉观测，先按记录划分冻结独立检验，再在同一成本和容差合同下比较查表、调度和成熟动态入流基线。不得用 TM89404 的 power-lever 图、D15 重叠窗口或旧开发曲线冒充这条记录。
