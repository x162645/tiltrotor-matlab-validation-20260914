function results=run_line_b_v7_speed_sweep_n18(outputRoot)
%RUN_LINE_B_V7_SPEED_SWEEP_V2 Damped Newton, crosschecked against V1.
...
P.rotor.correctionIdentity='CORRIGAN_POSITIVE_LIFT_WASHOUT_V4_N18';
...
identity='V7_SPEED_SWEEP_ZERO_IMMERSED_N18';
speeds=[40:5:105,35:-5:5,0.01,0];
config=struct('identity',identity,'speedOrder_kt',speeds,'solver','DAMPED_NEWTON_THEN_FMINSEARCH','solverRevision','V2', ...
...
frozenPath=fullfile(outputRoot,'V7_SWEEP_FROZEN_V2.mat');
% Exploratory n=1.8 variant: create its own frozen configuration, separate
% from the n=1 V1/V2 output. This variant is not a holdout model.
v1Path=fullfile(outputRoot,'V7_SWEEP_FROZEN.mat');
if exist(v1Path,'file')
 v1=load(v1Path,'P','hashes');
 assert(isequaln(v1.P,P)&&isequaln(v1.hashes(2:end),hashes(2:end)), ...
  'run_line_b_v7_speed_sweep_n18:PhysicalMismatch','Existing n18 output uses a different configuration.');
else
 save(v1Path,'P','config','contract','files','hashes');
end
if exist(frozenPath,'file')
 ...
 write_json(fullfile(outputRoot,'V7_SWEEP_FROZEN_V2.json'),manifest);
end
% The n=1.8 exploratory branch is not crosschecked against the n=1 runner;
% it has its own frozen configuration and is compared only after completion.
crosscheck=struct('passed',true,'purpose','exploratory documented n=1.8 variant; no n=1 solver equivalence claim');
if false
...
for k=1:numel(speeds)
 ...
 if exist(pointPath,'file')
  loaded=load(pointPath,'result');result=loaded.result;result.row=upgrade_row(result.row);
 ...
 elseif v==0||(v==0.01&&~all(ismember([5 10],[pool.speed_kt])))
  ...
  for j=1:numel(queue)
   ...
   if exist(attemptPath,'file')
    stored=load(attemptPath,'attempt');attempt=stored.attempt;attempt.row=upgrade_row(attempt.row);
   else
...
try
 a.row.solverRevision='NEWTON_V2_FAILED';
 [z,newtonOK,newtonTrace,newtonFailure]=damped_newton(seed);
 newtonEvaluations=evals;a.newtonTrace=newtonTrace;a.newtonFailure=newtonFailure;
 if ~isempty(newtonTrace),a.row.newtonIterations=newtonTrace(end).iteration;end
 a.row.newtonEvaluations=newtonEvaluations;
 if newtonOK
...
a.row.elapsed_s=toc(t0);a.row.invalidEvaluationCount=invalid;a.row.evaluationCount=evals;a.invalidIdentifiers=unique(ids);
 ...
 function [z,ok,trace,failure]=damped_newton(z)
...
r=struct('speed_kt',NaN,'speed_mps',NaN,'solverRevision','NOT_RUN', ...
 'newtonIterations',0,'newtonEvaluations',0,'fminsearchEvaluations',0,'numericallyAccepted',false,'status','NOT_RUN', ...
...
function r=upgrade_row(r)
if ~isfield(r,'solverRevision'),r.solverRevision='FMINSEARCH_V1';end
