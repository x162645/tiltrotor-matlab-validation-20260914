...
% Generated analysis copy has only diagnostic fields/message capture added.
% Original solver, residual, parameters, finite differences and guards retained.
...
section=replace_one(section,"z=z0(:); info=struct('converged',false,'iterations',0,'residualNorm',Inf);", ...
 "z=z0(:); info=struct('converged',false,'iterations',0,'residualNorm',Inf,'reason','ITERATION_LIMIT','condition',NaN,'z',z0(:),'vi',viNow);");
section=replace_one(section,'[res,scale]=flap_residual(z,viNow); rn=res/scale;', ...
 '[res,scale]=flap_residual(z,viNow); rn=res/scale; info.iterations=kk;info.residualNorm=norm(rn);info.z=z;info.rawResidual=res;info.scale=scale;');
...
S=readtable(fullfile(here,'original_baseline_trim_seeds.csv'));rows={};records={};count=0;
for speedKt=[60 100]
 ...
 % Existing fminsearch initial physical steps, plus negative counterparts.
 d=[2*pi/180;10*pi/180;1]*.00025;
 probes=[zeros(3,1),diag(d),-diag(d)];
 for j=1:size(probes,2)
  ...
  try
   [F,M,out]=line_b_instrumented_rotor(x,ctrl,0,-1,mp.cgShift,P);ok=true;
  catch ME,err=ME.identifier;end
  trace=evalin('base','LINE_B_LAST_FLAP_TRACE');
  if ok && j==1
   [F0,M0,o0]=m1_evidence_v1_forward_rotor(x,ctrl,0,-1,mp.cgShift,P);
...
[~,head]=system('git rev-parse HEAD');result=struct('head',strtrim(head),'table',A,'records',{records}, ...
 'version',version,'newTrimSearches',0,'probeRotorCalls',count,'identityRotorCalls',2,'solverChanged',false,'physicsChanged',false);
