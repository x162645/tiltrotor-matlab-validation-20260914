...
for k=1:3
 ...
 fixedRow=struct('speed_kt',speeds(k),'betaM_deg',rec.betaM_deg, ...
 ...
 if isempty(fixed),fixed=fixedRow;else,fixed(k)=fixedRow;end
 ...
 trimRow=struct('speed_kt',speeds(k),'nacelle_deg',30,'rpm',P.rotor.Omega*60/(2*pi), ...
 ...
 if isempty(trimRows),trimRows=trimRow;else,trimRows(k)=trimRow;end
