function result=run_line_b_v6_trim_case(speedKt,outputRoot)
% V6 = V5 plus source free-field angle and intrinsic-moment consistency.
% Initial guesses, optimizer and acceptance thresholds are identical to V5.
...
contract.identity='XV15_SHARED_FREEFIELD_CONTINUOUS_ROTOR_V6';contract.targetFitting=false;contract.productionPhysicsModified=true;
...
try
 ...
 writetable(row,fullfile(outputRoot,'V6_TRIM_POINT.csv'));disp(row);result.summary=row;
...
[~,h]=system('git rev-parse HEAD');result.commit=strtrim(h);save(fullfile(outputRoot,'V6_TRIM_RESULTS.mat'),'result','P');
meta=result;for name={'point','summary'},if isfield(meta,name{1}),meta=rmfield(meta,name{1});end,end
fid=fopen(fullfile(outputRoot,'V6_TRIM_MANIFEST.json'),'w');assert(fid>=0);c=onCleanup(@()fclose(fid));fwrite(fid,jsonencode(meta));clear c;
 ...
 function J=objective(yv)
  ...
  try
   ...
   if ~isfinite(J)||~isreal(J),error('run_line_b_v6_trim_case:NonfiniteCost','Invalid cost');end
