...
if nargin<5,sourceOverride='';end
...
if strcmp(sourceOverride,'legacy_surfaces')
 ...
 % This branch cannot be used for source-model accuracy claims.
 P.wing=rmfield(P.wing,'coefficientModel');
 if isfield(P.fuselage,'coefficientModel'),P.fuselage=rmfield(P.fuselage,'coefficientModel');end
 if isfield(P.htail,'modelIdentity'),P.htail=rmfield(P.htail,'modelIdentity');end
 if isfield(P,'interference'),P=rmfield(P,'interference');end
 identity='V8_CONTINUATION_LEGACY_SURFACE_GATE_DIAGNOSTIC';
end
Pbase=P;
...
for ia=1:numel(nacelleDeg)
 ...
 if betaDeg>=89.999 && ~strcmp(sourceOverride,'legacy_surfaces')
