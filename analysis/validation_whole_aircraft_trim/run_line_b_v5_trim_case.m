function result=run_line_b_v5_trim_case(speedKt,outputRoot)
% V5 = V4 plus source wing-pylon coefficient family. V1 solver unchanged.
...
P.rotor.correctionIdentity='CORRIGAN_POSITIVE_LIFT_WASHOUT_V4';P.wing.coefficientModel='GTRS_HELI_FLAP40_25_V5';
...
contract.identity='XV15_COHERENT_TAIL_CONTINUOUS_ROTOR_SOURCE_WING_V5';contract.targetFitting=false;contract.productionPhysicsModified=true;
contract.rotorIdentity='M1_CONTINUOUS_CORRIGAN_V4';contract.rotorCorrectionIdentity=P.rotor.correctionIdentity;contract.wingIdentity=P.wing.coefficientModel;
...
try
 ...
 writetable(row,fullfile(outputRoot,'V5_TRIM_POINT.csv'));disp(row);result.summary=row;
...
[~,h]=system('git rev-parse HEAD');result.commit=strtrim(h);save(fullfile(outputRoot,'V5_TRIM_RESULTS.mat'),'result','P');
meta=result;for name={'point','summary'},if isfield(meta,name{1}),meta=rmfield(meta,name{1});end,end
fid=fopen(fullfile(outputRoot,'V5_TRIM_MANIFEST.json'),'w');assert(fid>=0);c=onCleanup(@()fclose(fid));fwrite(fid,jsonencode(meta));clear c;
 ...
 function J=objective(yv)
  ...
  try
   ...
   if ~isfinite(J)||~isreal(J),error('run_line_b_v5_trim_case:NonfiniteCost','Invalid cost');end
