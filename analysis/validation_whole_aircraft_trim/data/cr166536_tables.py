...
def resolved_unit(table, raw):
    ...
    if table == "3-VIII":
        return "ft3"
    if table == "6/constants" and raw.get("parameter") in {"d_sigma_d_p_hat", "d_sigma_d_r_hat"}:
        return "dimensionless"
    return unit
...
class TableDatabase:
    def __init__(self, root=ROOT):
        ...
        for path in sorted(self.root.glob("CR166536*.csv")):
            ...
            self.files.append(dict(file=path.name, rows=len(rows), kind="constants" if "parameter" in rows[0] else "tables",
                                   sha256=hashlib.sha256(path.read_bytes()).hexdigest()))
