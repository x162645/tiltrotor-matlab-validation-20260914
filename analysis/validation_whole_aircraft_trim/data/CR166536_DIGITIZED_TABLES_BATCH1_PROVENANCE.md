...
- **数字化表**：
  ...
  - TABLE_1_II_ENDURANCE_SUPP：Table 1-II上半，`C_T/σ`耐久限制表；该表是限值标记，不参与主计算。
  - TABLE_024：Table 1-II下半，侧向飞行修正因子 `X_SF=f(|V̄|)`。
  - TABLE_025：Table 1-III，并列旋翼修正因子 `X_SS=f(|μ̄|)`。
  - TABLE_001/002/003：Table 2-Ia/Ib/Ic，旋翼尾迹对平尾诱导速度比 `W_i|_{R/H}/W_i`，分别 `β_m=0°,15°,30°`；TABLE_004（Table 2-Id，同B-24页）给出 `β_m=60°,90°` 时常数0.0。
