function result=run_line_b_tail_trim_case(speedKt,outputRoot,useArchivedInitials,variant)
% Prescribed case, unchanged V1 residual/tolerances/optimizer. Explicit V2/V3.
% Initials: original MODEL run34175347392; never GTRS target trim.
if nargin<3,useArchivedInitials=false;end
if nargin<4,variant='VELOCITY_ONLY_V2';end
...
if strcmp(variant,'COHERENT_TAIL_V3')
    P=line_b_coherent_tail_parameters(P);contract.identity=P.validation.identity;
elseif ~strcmp(variant,'VELOCITY_ONLY_V2')
    error('run_line_b_tail_trim_case:UnknownVariant','Explicit V2 or V3 required.');
end
contract.interactionSource='NASA_CR166536_SOURCE_COHERENT_SUBSET';
contract.targetFitting=false;contract.productionPhysicsModified=true;
contract.claimBoundary='GTRS_SOURCE_BASED_CORRELATION_NOT_INDEPENDENT_FLIGHT_VALIDATION';
seed=[theta(k)*d2r;collective(k)*d2r;stick(k)];scale=[2*d2r;10*d2r;1];
seedSource='Run34175347392_rounded_model';
if useArchivedInitials
    ...
    seedSource='Run34175347392_exact_outer_and_per_side_flap_initials';
end
...
result=struct('identity',contract.identity,'variant',variant,'speed_kt',speedKt,'cost',cost, ...
    ...
    'numericallyAccepted',false,'externalAccuracyPassed',false, ...
    'referenceRole','GTRS_REFERENCE_SIMULATION_NOT_FLIGHT');
try
    ...
    result.replayMeaning='Repeated evaluation of returned state; not independent trim search.';
    ...
    result.finalErrorIdentifier=ME.identifier;result.finalErrorMessage=ME.message;
    fprintf('FINAL_EVALUATION_FAILED %s: %s\n',ME.identifier,ME.message);
end
...
c=onCleanup(@() fclose(fid));fwrite(fid,jsonencode(meta),'char');clear c;
    function point=evaluate(zz)
        ...
        point=struct('x',x,'u',u,'xdot',xdot,'residual',xdot([1 3 5]),'eomOut',eom,'allocation',alloc);
    end
    function J=objective(yv)
        zz=seed+scale.*yv;
        if any(zz<bounds(:,1))||any(zz>bounds(:,2))
            violation=max(bounds(:,1)-zz,0)./(bounds(:,2)-bounds(:,1))+max(zz-bounds(:,2),0)./(bounds(:,2)-bounds(:,1));
        ...
        try
            ...
            if ~isfinite(J)||~isreal(J)
                invalidCount=invalidCount+1;invalidIds{end+1}='NONFINITE_OR_COMPLEX_COST';J=1e30;
            end
        catch ME
