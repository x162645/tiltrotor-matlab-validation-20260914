function result=run_line_b_v4_trim_case(speedKt,outputRoot)
% Explicit V4 coherent tail + continuous section correction. Same V1 solver.
% Current comparison is reference-guided model development, not blind validation.
...
contract.identity='XV15_COHERENT_TAIL_CONTINUOUS_ROTOR_V4';contract.targetFitting=false;contract.productionPhysicsModified=true;
...
try
 ...
 writetable(row,fullfile(outputRoot,'V4_TRIM_POINT.csv'));disp(row);result.summary=row;
...
[~,h]=system('git rev-parse HEAD');result.commit=strtrim(h);save(fullfile(outputRoot,'V4_TRIM_RESULTS.mat'),'result','P');
meta=result;for name={'point','summary'},if isfield(meta,name{1}),meta=rmfield(meta,name{1});end,end
fid=fopen(fullfile(outputRoot,'V4_TRIM_MANIFEST.json'),'w');assert(fid>=0);c=onCleanup(@()fclose(fid));fwrite(fid,jsonencode(meta));clear c;
 ...
 function J=objective(yv)
  ...
  try
   ...
   if ~isfinite(J)||~isreal(J),error('run_line_b_v4_trim_case:NonfiniteCost','Invalid cost');end
