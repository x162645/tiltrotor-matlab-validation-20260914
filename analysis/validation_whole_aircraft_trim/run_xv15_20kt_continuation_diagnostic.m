function results = run_xv15_20kt_continuation_diagnostic(outputRoot)
...
for k=1:numel(speeds)
    ...
    r=solve_one(c,seed,Pw,modelIdentity,220,'ones');
...
c=struct('name','XV15_HELI_20.00KT_MULTISTART','V',20*knot2mps,'betaM',0,'gamma',0);
...
for j=1:numel(seeds)
    rr=solve_one(c,seeds{j},Pw,modelIdentity,180,'ones');
    ms(j)=pack_ms(rr,j,seeds{j},d2r);
end
points=struct2table(rows); multistart=struct2table(ms);
writetable(points,fullfile(outputRoot,'XV15_20KT_CONTINUATION_POINTS.csv'));
writetable(multistart,fullfile(outputRoot,'XV15_20KT_MULTISTART.csv'));
...
writetable(summary,fullfile(outputRoot,'XV15_20KT_CONTINUATION_SUMMARY.csv'));
results=struct('contract',contract,'points',points,'multistart',multistart,'summary',summary,'reports',{reports});
save(fullfile(outputRoot,'XV15_20KT_CONTINUATION_RESULTS.mat'),'results');
